...
func (c client) isTokenExpired() bool {
	return c.impersonating.accessTokenExpires.Before(time.Now())
}

func (c client) isTokenAboutToExpire() bool {
	// Consider an access token "about to expire" if it's set to expire in less than 15m
	return !c.isTokenExpired() && c.impersonating.accessTokenExpires.After(time.Now().Add(time.Minute*time.Duration(-15)))
}
