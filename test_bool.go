...
func TestTransactionUpdateMeta(t *testing.T) {
  ...
  isLease := false
  metaUpdateReq := TransactionMetaUpdate{Data: &TransactionMeta{IsLease: &isLease}}
  _, err = transactionsClient.UpdateTransactionMeta("1075", metaUpdateReq)
  if err != nil {
    fmt.Println(err)
  }
  txn, err := transactionsClient.GetDetail("1075")
  if txn.IsLease != &isLease {
	t.Errorf("Is lease not set %t, want %t", *txn.IsLease, false)
  }
  ...
  txn, err = transactionsClient.GetDetail("1075")
  if txn.IsLease != &isLease {
	t.Errorf("Is lease not set %t, want %t", *txn.IsLease, true)
  }
}
