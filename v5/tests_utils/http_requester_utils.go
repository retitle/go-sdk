...
import (
	...
	"github.com/retitle/go-sdk/v5/core/mocks"
)
...
func Make200Response[T any](body *T) *http.Response {
	stringReadCloser := ParseStructToIoReadCloser[T](body)
	return &http.Response{StatusCode: http.StatusOK, Body: stringReadCloser}
}

func MakeResponseWithStatus[T any](status int, body *T) *http.Response {
	stringReadCloser := ParseStructToIoReadCloser[T](body)
	return &http.Response{StatusCode: status, Body: stringReadCloser}
}
