...
func AdaptRequest(r HttpRequestComponents) http.Request {
	requestPtr, err := http.NewRequest(r.Method, r.Url, ParseDirectStructToIoReadClose(r.Body))
	if err != nil {
...
type GlideExternalApiTestCase[T any] struct {
	Name                   string
	ArrangeCustom          func(client glide.Client)
...
func RunGlideExternalApiTestCases[T any](
	t *testing.T,
	testCases []GlideExternalApiTestCase[T],
) {
